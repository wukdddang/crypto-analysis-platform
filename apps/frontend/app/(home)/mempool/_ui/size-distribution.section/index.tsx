...
export default function SizeDistributionSection() {
  ...
  // Age Distribution과 동일한 그라데이션 색상 (작은 크기 -> 큰 크기 순서)
  const getSizeColor = (index: number, total: number) => {
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
          ...
          {sizeDistribution.map((item, index) => {
            ...
            const height =
              maxCount > 0
                ...
                : 0;
            const color = getSizeColor(index, sizeDistribution.length);
                ...
                <div className="w-full relative group cursor-pointer h-full flex items-end">
                  ...
                  <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-10">
                    <div className="bg-gray-900 dark:bg-gray-700 text-white text-xs rounded-lg p-2 whitespace-nowrap shadow-lg">
                      <div className="font-semibold">{item.size} bytes</div>
      ...
      <div className="mt-6 border-t border-gray-200 dark:border-gray-700 pt-4">
        <div className="text-sm text-gray-600 dark:text-gray-400 mb-3">
          Size Values (bytes) - Continuous Distribution
        </div>
          ...
          <div className="flex space-x-1">
            {Array.from({ length: 40 }, (_, i) => (
              <div
                ...
                style={{
                  backgroundColor: getSizeColor(i, 40),
                }}
              />
            ))}
          </div>
